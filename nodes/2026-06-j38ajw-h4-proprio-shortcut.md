...
type: experiment
status: completed
hypothesis: 'ROOT CAUSE (found by the h2-rehypothesize workflow, code-confirmed): the action head is fed
...
## Results
| dim | base | H4-PROP (proprio-xyz dropout 0.5) | Δ |
|---|---|---|---|
| clean | 1.00 (56/56) | 0.96 (53/55) | −0.04 |
| swap | 0.00 (0/30) | 0.00 (0/29) | 0.00 |
| task | 0.29 (10/34) | 0.27 (9/33) | −0.02 |
(eval stopped at ~30 ep/dim — swap 0/29 conclusive; user pivoted to H5.)

## Conclusion
**NULL — and an informative one: proprio is NOT the shortcut.** Dropping 50% of the eef-xyz proprio
did NOT move swap off 0.00 AND did not hurt clean (0.96). Two consequences: (1) the position-
memorization shortcut does not live (solely) in proprio; (2) the model does not even need eef-xyz for
clean accuracy. By elimination the shortcut sits in the **vision patch-index** (object always at the
same image location → memorized patch→action), OR the action objective itself admits a position
solution. Rather than chase pixel hacks, pivot to **mathematically-principled H5**: enforce the
symmetry the swap benchmark actually measures (position-EQUIVARIANCE) / a conditional-MI objective that
forces the ACTION to depend on visual object location. See new node(s) h5-*.
